...
class DBManager:
    ...
    def save_profile(self, profile_name, os_name, packages, env_vars, symlinks):
        try:
            packages_str = ",".join([
                f"{pkg['name']}:{pkg['version']}:{pkg['repo_url']}:{pkg['download_url']}"
                for pkg in packages
            ])
                ...
                packages = [
                    {
                        ...
                        'repo_url': fields[2] if len(fields) > 2 else '',
                        'download_url': fields[3] if len(fields) > 3 else '',
                    }
                    for fields in (pkg.split(':') for pkg in profile.packages.strip().split(','))
                ]
                env_vars = {}
